...
from .mask_iou import MaskIoUFeatureExtractor
...
class MultiScaleMaskedTransformerDecoder(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        # self.iou_head = MaskIoUFeatureExtractor(hidden_dim+1,hidden_dim,1)
        self.iou_head = MLP(hidden_dim,hidden_dim,1,3)
        self.object_embed = MLP(hidden_dim, hidden_dim, 2, 3)
    ...
    def forward(self, x, mask_features, dense_feature,mask = None ):
        ...
        outputs_object,outputs_mask, attn_mask,output_iou,output_embed = self.forward_prediction_heads(output, mask_features,dense_feature, attn_mask_target_size=size_list[0])
        # outputs_object,  outputs_mask, attn_mask = self.forward_prediction_heads(output, mask_features, attn_mask_target_size=size_list[0],text_classifier=text_classifier, num_templates=num_templates)
        ...
        for i in range(self.num_layers):
            ...
            outputs_object,outputs_mask, attn_mask,output_iou,output_embed = self.forward_prediction_heads(output, mask_features, dense_feature,attn_mask_target_size=size_list[(i + 1) % self.num_feature_levels])
            # outputs_object, outputs_class,outputs_mask, attn_mask = self.forward_prediction_heads(output, mask_features, attn_mask_target_size=size_list[(i + 1) % self.num_feature_levels],text_classifier=text_classifier, num_templates=num_templates)
            predictions_object.append(outputs_object)
            # predictions_class.append(outputs_class)
            predictions_mask.append(outputs_mask)
            predictions_iou.append(output_iou)
            predictions_embed.append(output_embed)
        ...
        out = {
            ...
            # 'pred_embds': self.decoder_norm(predictions_embed[-1]),
            'aux_outputs': self._set_aux_loss(
                predictions_object if self.mask_classification else None, predictions_iou,predictions_mask
            )
        }
        return out
    ...
    def forward_prediction_heads(self, output, mask_features,dense_feature, attn_mask_target_size):
        ...
        mask_for_pooling = F.interpolate(outputs_mask, size=dense_feature.shape[-2:],mode='bilinear', align_corners=False)
    ...
    def _set_aux_loss(self, outputs_object, outputs_mask_iou,outputs_seg_masks):
        ...
        if self.mask_classification:
            return [
            {"pred_object_logits": a,  'pred_ious':b, "pred_masks": c}
                for a, b, c in zip(outputs_object[:-1], outputs_mask_iou[:-1],outputs_seg_masks[:-1])
            ]
        else:
            return [{"pred_masks": b} for b in outputs_seg_masks[:-1]]